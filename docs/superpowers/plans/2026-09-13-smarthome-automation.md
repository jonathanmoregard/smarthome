...
- [ ] Include missed-reset startup tests before and after 04:00, including first-start reset-marker initialization before commands are accepted. A due reset atomically moves every frozen control into smooth convergence; `Converging` is already unfrozen and becomes `Follow` when convergence completes.
